...
`unified-canvas-options-plan.md` Q1). Extract to `@invana/canvas-state` **only if** a second
...
2. **Packaging**: `state/` module in `@invana/canvas` now (recommended) vs. `@invana/canvas-state` package.
