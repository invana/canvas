...
- [node-record-types-rfc.md](./node-record-types-rfc.md) — **two record types, not five** (📋 proposed; half already true). Inventories what `GraphNode` / `NodeOption` / `NodeData` / `NodeInput` / `GraphDataOptions` each claimed to do and which had users — `NodeData`'s own TSDoc called itself *"the record `GraphStore` holds"* while the store demonstrably holds `GraphNode`, and it had **zero** runtime users; 112 story files had imported it anyway because it was exported and structurally compatible. Names the distinction the old design conflated: **two resolver moments** — *layer-template* resolvers (`NodeOption.style`, fire every render against the stored node, ✅ built, this is how `ColorByBehaviour` works) versus *per-record input* resolvers (fire once at insert against raw data, ❌ never implemented). Target is one resolver type + one stored type, which deleting the dead three already achieved. Remaining work is the **leak**: `GraphNode.style` is `unknown` where `NodeData.style` was `NodeStyle`, so the type is strictly weaker on its most-read field and three stories now carry casts — and the stated reason (a store→layer cycle) doesn't hold, since type-only imports are erased. Recommends extracting `NodeStyle` to its own module. 5 work items, the largest being a doc sweep: `packages/graph/CLAUDE.md` still documents the deleted split as current.
- [required-type-on-records-rfc.md](./required-type-on-records-rfc.md) — **`type` becomes required on `GraphNode` / `GraphEdge`** (🚧 core landed, migration pending). **One type, `type: string`, every author writes it** — the separate-input-type alternative was built, measured and reverted: it migrates for nearly free but means two types to keep straight for one field. Correcting the draft's own scare number: the headline "24,683 records" is the *record* count, not the *edit* count — `h1b2019`, `flare`, `life-tree` and `lattice` have **zero literal node lines** (all loop-generated), so they're ~2 edits each. Real shape: a dozen dataset generators at one line each, plus **2,546 literal fixture sites** across 232 storybook files (median 7/file). `UNKNOWN_TYPE` ships as a named export and the store still normalises with **`||` not `??`** on both insert *and* update paths — now as a runtime net for `importData`, parsed JSON and `Partial` patches, all of which bypass the compiler. **§9 is the migration plan**: revert → datasets get *real* types (`flare`/`h1b2019`/`life-tree` already carry `isLeaf`/`depth`, so a one-line ternary gives them a genuine partition where they had none) → ts-morph codemod driven by `TS2741` diagnostics rather than regex → reader cleanup. Also: what `'unknown'`-as-a-value changes (§4), the dead-type-cluster deletion (§6, done), and all six serialization paths traced (§7) — old snapshots need no migration, new ones omit `type` when it equals the sentinel.
