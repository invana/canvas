...
| `setDecoration` (15 sites) · `setBadge` · `removeBadge` · `getDecoration` · `setDecorationVisible` | **(a)** — decoration *styles* and badges joined the spec vocabulary in `839d1a2`, but **attachment is still imperative**. This is the single largest remaining block. ⚠ **Settle the shape first:** `NodeStyle.decorations` already ships as a *discriminated-union array*, while modelling attachment as `Record<slot, DecorationSpec>` would make it a *slot dict*. Two vocabularies for the same thing is the failure mode — see D-B in [`data-types-implementation-plan.md`](./data-types-implementation-plan.md) | 📋 the big one; own step |
