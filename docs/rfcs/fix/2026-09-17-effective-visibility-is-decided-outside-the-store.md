...
| **Row status** | **landed 4** · implemented 1 · rejected 2 — 7 rows. The RFC is **not landed**: F6's story checks (V2 · V3) have not been run |
...
| V6 | **control** · pending | Authored positions + a layout | `story:designs/AgenticWorkflow` | Frame-identical; the placement term never engages | F1, X5 |
| V7 | **control** · pending | No `activeLayout` at all | `story:canvas-react/Canvas/Basic` | Frame-identical — without a layout, `(0,0)` is the honest position (D4) | F1 |
| V8 | pending | `activeLayout` naming an unregistered layout | scratch | Graph paints within `PLACEMENT_GRACE_MS`, not never | F2 |
| V9 | pending | A first run ending `'stopped'` | scratch | Term still lifts, via the run or the floor | F2, U2 |
