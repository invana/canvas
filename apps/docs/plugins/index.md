...
Decorations register into the `PrimitivesRenderer`. Layers mount onto the `Canvas` via `canvas.layers.add(...)`. Behaviours register via `canvas.behaviours.add(...)` — none of them auto-enable; every behaviour is explicitly opt-in.
