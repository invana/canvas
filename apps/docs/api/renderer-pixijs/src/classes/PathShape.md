...
to `(spec.x, spec.y)`, exactly like [PolygonShape](PolygonShape.md).
...
- [`ShapeBase`](ShapeBase.md)\<`PathSpec`\>

## Constructors

### Constructor

> **new PathShape**(`spec`, `host`): `PathShape`
...
> `readonly` `static` **kind**: `"path"` = `'path'`
...
> `static` **scaleSpec**(`spec`, `factor`): `Partial`\<`PathSpec`\>
