 ...
 *  Row 2  Polygon (hex) · Diamond · Star · Hexagon
 *
 * Demonstrates:
 *   - `addSolid(type, spec)` API
 *   - All 7 built-in solid types (including the new `hexagon`)
...
const COLORS = ['#4fc3f7', '#81c784', '#ffb74d', '#f06292', '#ce93d8', '#4dd0e1', '#a5f3fc'];
const c = (i: number) => COLORS[i % COLORS.length] as string;

const G = 170;   // grid cell size
const ROW1 = -G / 2;
const ROW2 =  G / 2 + 30;
...
export const SolidElements: Story = {
  ...
  play: async () => {
    ...
    // ── Row 2 ──────────────────────────────────────────────────────────────
    const r2y = ROW2 + 60;
    const r2xs = [-G * 1.5, -G * 0.5, G * 0.5, G * 1.5];

    elements.addSolid('polygon', {
      id: 'polygon', x: r2xs[0], y: r2y, radius: 50, sides: 6,
      label: 'Polygon (6)',
    ...
    elements.addSolid('diamond', {
      id: 'diamond', x: r2xs[1], y: r2y, radius: 52,
      label: 'Diamond',
    ...
    elements.addSolid('star', {
      id: 'star', x: r2xs[2], y: r2y, radius: 52,
      label: 'Star',
    ...
    elements.addSolid('hexagon', {
      id: 'hexagon', x: r2xs[3], y: r2y, radius: 50,
      label: 'Hexagon',
      style: { fill: c(6), stroke: '#ffffff', strokeWidth: 2 },
      interactive: true,
