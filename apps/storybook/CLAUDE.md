...
### `canvas-ui/*` stories — only three buckets: `apps` / `editors` / `views`

Keep the `@invana/canvas-ui` sidebar simple: **every canvas-ui story is either an editor or a view** (plus the composed `apps`). There are exactly three top-level folders under `stories/canvas-ui/` — don't invent a fourth (no `visibility/`, `node-templates/`, `panels/`, etc.):
