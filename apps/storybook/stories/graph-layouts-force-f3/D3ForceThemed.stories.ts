...
export const D3ForceThemed: Story = {
  render: () => createContainer({ id: 'graph-d3-force-themed' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    // ── Themed background sits behind the graph ─────────────────────────
    ...
    // ── Graph layer with theme-aware edge stroke ────────────────────────
    const lightEdge = 0xcbd5e1;
    const darkEdge = 0x475569;

    const graph = new GraphLayer({
      id: 'graph',
      options: { edgeDefaults: { strokeWidth: 1, arrow: false } },
    });
