...
const generateGraphData = (): CanvasData => ({
  nodes: [
    ...
    { id: 'n2', x: 100, y: 0, shape: 'roundedRect', width: 80, height: 50, label: 'Or Me', style: { fill: '#50c878', stroke: '#333', strokeWidth: 2 } },
...
const createSelectionDemo = (): HTMLElement => {
  ...
  requestAnimationFrame(async () => {
    const canvas = new Canvas({
      ...
      data: {
        nodes: [
          ...
          { id: 'n4', x: -75, y: 60, shape: 'roundedRect', width: 80, height: 50, label: 'Node 4', style: { fill: '#ffd93d', stroke: '#333', strokeWidth: 2 } },
          { id: 'n5', x: 75, y: 60, shape: 'roundedRect', width: 80, height: 50, label: 'Node 5', style: { fill: '#9b59b6', stroke: '#333', strokeWidth: 2 } },
        ],
        edges: [
    ...
    const deselectNode = (id: string) => {
      ...
      if (original) {
        canvas.updateNode(id, { style: original });
      }
      updateSelectionInfo();
    };
