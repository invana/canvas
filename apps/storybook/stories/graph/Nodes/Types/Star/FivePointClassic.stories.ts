...
const meta: Meta = { title: 'graph/Nodes/Types/Star/FivePointClassic' };
export default meta;
type Story = StoryObj;

/**
 * **Classic 5-point star** variant (`points: 5`, inner/outer ratio 0.4 —
 * the canonical "school-sticker" star). Shown across the resting
 * `default` plus the five canonical interaction states (`hovered`,
 * `selected`, `highlighted`, `dimmed`, `disabled`).
 */
export const FivePointClassicStory: Story = {
  name: 'FivePointClassic',
  render: () => createContainer({ id: 'graph-node-types-star-5-classic' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-star-5-classic')!;
    ...
    const canvasOptions = {
      layers: {
        graph: {
          node: {
            style: {
              shape: { kind: 'star', points: 5, innerRadius: 22, outerRadius: 55 },
              bgFill: 0x3b82f6,
