...
const meta: Meta = { title: 'graph/Nodes/Types/Star/FivePointPointy' };
export default meta;
type Story = StoryObj;

/**
 * **Pointy 5-point star** variant (`points: 5`, inner/outer ratio
 * 0.25). The tight inner radius gives long, sharp tips. Shown across the
 * resting `default` plus the five canonical interaction states
 * (`hovered`, `selected`, `highlighted`, `dimmed`, `disabled`).
 */
export const FivePointPointyStory: Story = {
  name: 'FivePointPointy',
  render: () => createContainer({ id: 'graph-node-types-star-5-pointy' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-star-5-pointy')!;
    ...
    const canvasOptions = {
      layers: {
        graph: {
          node: {
            style: {
              shape: { kind: 'star', points: 5, innerRadius: 13.75, outerRadius: 55 },
              bgFill: 0x3b82f6,
