...
/**
 * **The Tasks Panel — drafting a plan on the canvas.** The design's *"6 ·
 * drafting on the canvas — refused before it is published"* (`PlanDraftCanvas`):
 * the read-only plan of the sibling `PlanFlow` story, now being edited, with two
 * cards that exist only while a draft is open.
 *
 * - **The empty slot.** `+ add a task` is the card grammar with its outline
 *   dropped to the `divider` role — a hole in the picture, on the same 146px
 *   module so nothing reflows when it is filled. Its second row says what to do
 *   with it rather than naming a bound.
 * - **The refusal, mid-drag.** `bulk_write` is being dragged in from the
 *   catalogue — lifted off the grid, a soft shadow under it, outlined in
 *   destructive red because it spends `graph_write`, a bound the Loader envelope
 *   does not grant. The refusal is on the card at the moment of the drop, not in
 *   a dialog after publish: *refused before it is published, not at 2am*.
 * - **`fetch_source` keeps the `accent` outline** — it is the task the draft
 *   panel is editing (the retry clause being added to it).
 *
 * The shadow is a `glow` decoration: the engine has no drop-shadow primitive,
 * and a black glow at low alpha is the nearest honest equivalent of the design's
 * `box-shadow: 0 8px 22px rgb(0 0 0 / .18)`. The slot's border is solid because
 * `FreeformStructure.strokeWidth` has no dash counterpart.
 *
 ...
 * serialisable are the `<GraphCanvas>` tree and its behaviours.
 */
const meta: Meta = { title: 'usecases/by-casestudies/tasks-panel/PlanDraftFlow' };
export default meta;
type Story = StoryObj;
...
const STRUCTURES: NodeStructureRegistry = {
  ...
  bulk_write: {
    ...
    elements: [
      { id: 'name', type: 'text', x: 10, y: -4, bind: 'data.name', fontSize: 12, colorRole: 'heading' },
      { id: 'bound', type: 'text', x: 10, y: 18, bind: 'data.bound', fontSize: 10, color: 0xe05252 }
    ]
  }
};
...
const TYPES: NodeTypeRegistry = {
  ...
  add_task: { structure: 'add_task', styling: '', bindings: {} },
  bulk_write: { structure: 'bulk_write', styling: '', bindings: {} }
};
...
const DATA: GraphData = {
  nodes: [
    ...
    {
      // Lifted off the grid mid-drag, so it is placed by its own top-left
      // (102, 340) over a 48px card rather than on the 68px module.
      id: 'bulk_write',
      type: 'bulk_write',
      data: {
        name: 'bulk_write',
      ...
      style: {
        // The engine has no drop-shadow primitive; a black glow at low alpha is
        // the nearest equivalent of the design's lifted `box-shadow` on the drag.
        decorations: [{ kind: 'glow', id: 'lift', color: 0x000000, strokeWidth: 14, innerAlpha: 0.18 }]
      }
    }
  ],
  edges: [
...
export const PlanDraftFlow: Story = {
  render: () => (
