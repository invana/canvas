...
/**
 * **The Tasks Panel — the plan dashboard.** The design's *"D5 · the plan
 * dashboard — the flow, its arguments, how it has behaved"* (`PlanDash`): the
 * same eight tasks on the same grid, with the third row given over to
 * **behaviour across 14 runs** rather than configuration or a single outcome.
 *
 * - **The detail row becomes a median.** `p50 3.4s · 14/14` reads as what this
 *   task costs and how often it completes — the flow doubles as the performance
 *   table, so there is no second chart to reconcile with it.
 * - **One dot, and it is a warning.** `fetch_source` succeeded 12 of 14 times,
 *   so it alone carries a status dot. Every other task is unremarkable and says
 *   nothing; the exception is the whole message.
 * - **`triage` stays quiet** — it ran twice in fourteen. Real, but not part of
 *   the spine.
 * - **`announce` holds the warning outline** and reports the median wait on a
 *   human: `held p50 1m`.
 *
 ...
 * serialisable are the `<GraphCanvas>` tree and its behaviours.
 */
const meta: Meta = { title: 'usecases/by-casestudies/tasks-panel/PlanDashboardFlow' };
export default meta;
type Story = StoryObj;
...
const STRUCTURES: NodeStructureRegistry = {
  ...
  fetch_source: {
    ...
    strokeWidth: 1,
    elements: [
      { id: 'dot', type: 'circle', x: 10, y: 12, radius: 3.5, fill: 0xce8509 },
...
export const PlanDashboardFlow: Story = {
  render: () => (
