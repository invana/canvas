...
/**
 * **The Tasks Panel — a run, status as it ran.** The flow canvas the design
 * calls *"D1 · the run dashboard — the flow with status, the budget,
 * result.json"*. The same eight-task picture carries four artboards (`RunDash`,
 * `Main`, `DrawerRunAll`, `DrawerRunDebug`) — this story is that one flow.
 *
 * - **Every task carries its bound.** The 6px swatch beside the bound name is
 *   the only place a reader learns what a task may spend — `ingest` blue,
 *   `network` amber, `none` grey, `work_write` red. A run that spends nothing
 *   dangerous is legible without reading a word.
 * - **A dot means it ran.** `triage` has none: the branch was never taken. The
 *   design drops that card to 45%; template elements carry no alpha, so it goes
 *   quiet by *role* instead — its name falls from `heading` to `muted`.
 * - **The exception gets the border.** `announce` held for approval, so it is
 *   the one card outlined in warning, with a warning detail line to match.
 *
 ...
 * serialisable are the `<GraphCanvas>` tree and its behaviours.
 */
const meta: Meta = { title: 'usecases/by-casestudies/tasks-panel/RunFlow' };
export default meta;
type Story = StoryObj;
...
const STRUCTURES: NodeStructureRegistry = {
  check_bundle: {
    ...
    strokeWidth: 1,
    elements: [
      { id: 'dot', type: 'circle', x: 10, y: 12, radius: 3.5, fill: 0x1da54f },
      { id: 'name', type: 'text', x: 23, y: -4, bind: 'data.name', fontSize: 12, colorRole: 'heading' },
  ...
  fetch_source: {
    ...
    elements: [
      ...
      { id: 'bound', type: 'text', x: 22, y: 18, bind: 'data.bound', fontSize: 10, colorRole: 'muted' },
      { id: 'chip-ring', type: 'rect', x: 67.2, y: 27, width: 58.4, height: 14, fill: 0xce8509 },
      { id: 'chip-body', type: 'rect', x: 68.2, y: 28, width: 56.4, height: 12, fillRole: 'cardBg' },
      { id: 'chip-text', type: 'text', x: 71.2, y: 18, text: 'attempt 2', fontSize: 10, color: 0xce8509 },
      { id: 'detail', type: 'text', x: 10, y: 35.5, bind: 'data.detail', fontSize: 10.5, colorRole: 'muted' }
    ]
  },
  validate_records: {
  ...
  import_dataset: {
    ...
    elements: [
      ...
      { id: 'swatch', type: 'rect', x: 10, y: 31, width: 6, height: 6, fill: 0x0b73da },
      { id: 'bound', type: 'text', x: 22, y: 18, bind: 'data.bound', fontSize: 10, colorRole: 'muted' },
      { id: 'chip-ring', type: 'rect', x: 61.6, y: 27, width: 47.2, height: 14, fillRole: 'divider' },
      { id: 'chip-body', type: 'rect', x: 62.6, y: 28, width: 45.2, height: 12, fillRole: 'cardBg' },
      { id: 'chip-text', type: 'text', x: 65.6, y: 18, text: '3 lanes', fontSize: 10, colorRole: 'muted' },
      { id: 'detail', type: 'text', x: 10, y: 35.5, bind: 'data.detail', fontSize: 10.5, colorRole: 'muted' }
    ]
  },
  verify_counts: {
  ...
  triage: {
    ...
    strokeWidth: 1,
    elements: [
      { id: 'name', type: 'text', x: 10, y: -4, bind: 'data.name', fontSize: 12, colorRole: 'muted' },
  ...
  render: () => (
    ...
    <div style={{ width: '100%', height: '100dvh' }}>
      <GraphCanvas autoResize config={CONFIG}>
        ...
        {/* The flow *arrives* rather than cutting from blank to complete: each
            card fades in behind the one to its left, and each arrow behind the
            later of the two cards it joins. Plays once, when ELK reports a
            settled run — the same moment the engine's auto-fit frames the graph
            — then retires the effects it wrote, so nothing transient survives
            into the layer's data. Mounted after the layer it animates, like the
            layout above it. */}
        <EntranceBehaviour id="entrance" targetLayerId="graph" staggerMs={40} order="x" enabled />
      </GraphCanvas>
    </div>
  )
};
