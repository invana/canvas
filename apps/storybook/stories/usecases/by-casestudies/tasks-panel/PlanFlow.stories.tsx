...
/**
 * **The Tasks Panel — a plan, read-only.** The design's *"5 · a plan — the flow
 * it will run, read-only"* (`PlanDetail`; the same picture carries
 * `FilterAndSearch` and `PlanRetire`). This is the *definition*, not a run, and
 * the card grammar says so by what it omits:
 *
 * - **No status dots.** Nothing has happened yet. A plan showing dots would be
 *   claiming an outcome it cannot have.
 * - **The third row is configuration, not a result.** `map_over: datasets`,
 *   `when: counts differ`, `approval before it writes` — each is a clause of the
 *   plan, so a reader can tell what the flow *will* do without opening the
 *   manifest. Tasks with nothing to declare render a two-row card.
 * - **The bound swatch still carries the envelope**, the same channel it carries
 *   in a run — which is what makes a refusal explicable: a task is legal or not
 *   by the colour beside its name.
 * - **`announce` keeps the warning outline** because it holds for approval — a
 *   property of the plan, not of any one run.
 *
 ...
 * serialisable are the `<GraphCanvas>` tree and its behaviours.
 */
const meta: Meta = { title: 'usecases/by-casestudies/tasks-panel/PlanFlow' };
export default meta;
type Story = StoryObj;
 ...
 *
 * **The entrance.** Each connector carries a one-shot `reveal-connector`
 * decoration: the line draws itself on from source to target, and the `delayMs`
 * values stagger them along the flow so the diagram assembles in reading order
 * rather than appearing whole. `holdAtFull` defaults to `true`, so each edge
 * hands off to its own stroke when the sweep ends and nothing is left running.
 *
 * The **cards do not animate** — that needs the `fade-in` effect to be reachable
 * per node, and `GraphLayer` does not project `style.effects` yet (row C7 of
 * `docs/rfcs/feat/2026-09-17-a-graph-has-no-entrance-on-first-load.md`).
 * Decorations are wired end to end, which is why the edges can arrive today.
 */
  ...
  edges: [
    {
      ...
      target: 'fetch_source',
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 200 }] }
    },
    {
      id: 'e2',
      source: 'fetch_source',
      target: 'validate_records',
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 310 }] }
    },
    {
      id: 'e3',
      ...
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 420 }] }
    },
    {
      id: 'e4',
      ...
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 530 }] }
    },
    ...
    // It draws last, after the spine it branches off.
    {
      id: 'e5',
      source: 'triage',
      target: 'verify_counts',
      ...
      style: {
        strokeAlpha: 0.6,
        strokeDashArray: [5, 4],
        arrowTargetAlpha: 0.6,
        decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 750 }]
      }
    },
    {
      id: 'e6',
      source: 'verify_counts',
      target: 'import_report',
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 640 }] }
    },
    {
      id: 'e7',
      source: 'announce',
      target: 'import_report',
      type: 'THEN',
      style: { decorations: [{ kind: 'reveal-connector', id: 'entrance', durationMs: 260, delayMs: 750 }] }
    }
  ]
};
