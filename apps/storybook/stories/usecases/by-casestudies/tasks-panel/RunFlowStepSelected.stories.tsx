...
/**
 * **The Tasks Panel — a run, with a task picked.** The design's *"3 · a run —
 * its tasks drawn as the flow that ran"* (`RunDetail`): the run of the sibling
 * `RunFlow` story with `fetch_source` selected, so the panel beside it can show
 * that step's detail.
 *
 * Selection is **one channel and one card** — the outline takes the `accent`
 * role and nothing else moves. The design deliberately doesn't dim the rest,
 * scale the card, or add a halo: the picture still has to read as the run with
 * a selection in it, not as a selection with a run behind it. Taking the theme's
 * accent rather than a fixed green also means the selection matches whatever
 * family the host app is on.
 *
 ...
 * serialisable are the `<GraphCanvas>` tree and its behaviours.
 */
const meta: Meta = { title: 'usecases/by-casestudies/tasks-panel/RunFlowStepSelected' };
export default meta;
type Story = StoryObj;
...
export const RunFlowStepSelected: Story = {
  render: () => (
