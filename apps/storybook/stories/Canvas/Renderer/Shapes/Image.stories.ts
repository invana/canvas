...
export const Image: Story = {
  ...
  play: async ({ canvasElement }) => {
    class RenderLayer extends WorldLayer<{ textureRegistry?: TextureRegistry }> {
      renderer!: ShapesRenderer;
      protected createState() { return {}; }
      protected onMount(ctx: CanvasContext) {
        this.renderer = new ShapesRenderer({ container: this.container, camera: ctx.camera, textureRegistry: this.options.textureRegistry });
      }
      hitTest() { return null; }
    }
    ...
    const layer = new RenderLayer({ id: 'img-layer', options: { textureRegistry: registry } });
