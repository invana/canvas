...
const meta: Meta = { title: 'Canvas/Renderer/Shapes/Path' };
export default meta;
type Story = StoryObj;

export const Path: Story = {
  render: () => createContainer({ id: 'cvs-renderer-shape-path' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-renderer-shape-path')!;
    const canvas = new Canvas();
    await canvas.init({ container, autoResize: true });
    canvas.behaviours.register(new DragPanBehaviour({ id: 'pan', enabled: true }));
    canvas.behaviours.register(new WheelZoomBehaviour({ id: 'zoom', enabled: true }));

    const layer = new RenderLayer({ id: 'path', options: {} });
    ...
    const gui = new GUI({ title: 'Path (arrow)' });
