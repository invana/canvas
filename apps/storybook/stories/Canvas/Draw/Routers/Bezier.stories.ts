...
const meta: Meta = { title: 'Canvas/Draw/Routers/Bezier' };
export default meta;
type Story = StoryObj;

export const Bezier: Story = {
  render: () => createContainer({ id: 'cvs-router-bezier' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-router-bezier')!;
    ...
    const layer = new DrawLayer({ id: 'router-bezier-layer', options: {} });
    canvas.layers.add(layer);
    const g = layer.createGraphics('router-bezier-gfx');
    ...
    function redraw() {
      ...
      const polyline = draw.bezierRouter(SRC, TGT);
      g.clear();
    ...
    const gui = new GUI({ title: 'Bezier router' });
