...
const meta: Meta = { title: 'Canvas/Primitives/Connectors/PathStyles/Bezier' };
export default meta;
type Story = StoryObj;

/**
 * The `bezier` pathStyle draws a single cubic Bézier between the first and
 * last polyline points with auto-generated control handles (direction-aware
 * s-curve). Intermediate points are ignored — pick `smooth` for waypoint
 * routing. All settings are in the gui.
 */
export const Bezier: Story = {
  render: () => createContainer({ id: 'cvs-prim-pathstyle-bezier' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-prim-pathstyle-bezier')!;
    const canvas = new Canvas();
    await canvas.init({ container, autoResize: true });
    canvas.behaviours.register(new DragPanBehaviour({ id: 'pan', enabled: true }));
    canvas.behaviours.register(new WheelZoomBehaviour({ id: 'zoom', enabled: true }));

    const layer = new RenderLayer({ id: 'pathstyle-bezier', options: {} });
    ...
    const draw = (): void => {
      ...
      layer.renderer.addConnector('curve', {
        ...
        pathStyle: 'bezier',
        pathStyleOpts: { axis: settings.axis, tension: settings.tension },
    ...
    const gui = new GUI({ title: 'bezier pathStyle' });

    const pathStyleFolder = gui.addFolder('pathStyle (bezier)');
    pathStyleFolder.add(settings, 'axis', ['auto', 'h', 'v']).onChange(draw);
    pathStyleFolder.add(settings, 'tension', 0, 1, 0.01).onChange(draw);
