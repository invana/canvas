...
const meta: Meta = { title: 'Canvas/Primitives/Connectors/PathStyles/Smooth' };
export default meta;
type Story = StoryObj;

/**
 * The `smooth` pathStyle draws a Catmull-Rom spline through every polyline
 * point as cubic Béziers — the curve passes through every input point
 * exactly. Great for waypoint-driven curves. All settings are in the gui.
 */
export const Smooth: Story = {
  render: () => createContainer({ id: 'cvs-prim-pathstyle-smooth' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const layer = new RenderLayer({ id: 'pathstyle-smooth', options: {} });
    ...
    const draw = (): void => {
      ...
      layer.renderer.addConnector('curve', {
        ...
        pathStyle: 'smooth',
        pathStyleOpts: { tension: settings.tension },
