...
const meta: Meta = { title: 'Canvas/Primitives/Connectors/Routers/Orth' };
export default meta;
type Story = StoryObj;

/**
 * The `orth` router produces clean H/V segments — no obstacle awareness.
 * Honors source/target tangents (Z-bend when both align). Drag the
 * endpoints to move them; the path re-routes. All settings are in the gui.
 */
export const Orth: Story = {
  render: () => createContainer({ id: 'cvs-prim-router-orth' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-prim-router-orth')!;
    const canvas = new Canvas();
    await canvas.init({ container, autoResize: true });
    canvas.behaviours.register(new DragPanBehaviour({ id: 'pan', enabled: true }));
    canvas.behaviours.register(new WheelZoomBehaviour({ id: 'zoom', enabled: true }));

    const layer = new RenderLayer({ id: 'router-orth', options: {} });
    ...
    layer.renderer.addShape('a', {
      kind: 'rect', x: -180, y: -100, width: 80, height: 50,
    ...
    const gui = new GUI({ title: 'orth router' });
