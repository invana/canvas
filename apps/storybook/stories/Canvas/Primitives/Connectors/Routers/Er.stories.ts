...
const meta: Meta = { title: 'Canvas/Primitives/Connectors/Routers/Er' };
export default meta;
type Story = StoryObj;

/**
 * The `er` router exits each endpoint perpendicular to its boundary
 * (using the tangent set by the `boundary` or `perpendicular` anchor),
 * then routes orthogonally between the stub points. All settings are in
 * the gui.
 */
export const Er: Story = {
  render: () => createContainer({ id: 'cvs-prim-router-er' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-prim-router-er')!;
    const canvas = new Canvas();
    await canvas.init({ container, autoResize: true });
    canvas.behaviours.register(new DragPanBehaviour({ id: 'pan', enabled: true }));
    canvas.behaviours.register(new WheelZoomBehaviour({ id: 'zoom', enabled: true }));

    const layer = new RenderLayer({ id: 'router-er', options: {} });
    ...
    layer.renderer.addShape('a', {
      kind: 'rect', x: -180, y: 0, width: 100, height: 60,
    ...
    const draw = (): void => {
      ...
      layer.renderer.addConnector('edge', {
        kind: 'connector',
        router: 'er',
        routerOpts: { stubLength: settings.stubLength },
    ...
    const gui = new GUI({ title: 'er router' });

    const routerFolder = gui.addFolder('router (er)');
    routerFolder.add(settings, 'stubLength', 0, 80, 1).onChange(draw);
