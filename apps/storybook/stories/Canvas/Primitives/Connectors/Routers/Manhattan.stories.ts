...
const meta: Meta = { title: 'Canvas/Primitives/Connectors/Routers/Manhattan' };
export default meta;
type Story = StoryObj;

/**
 * The `manhattan` router routes H/V segments AROUND obstacles via A* on a
 * coarse grid. Drag any yellow obstacle to move it; the path re-routes live.
 * The gui exposes every knob: router opts, pathStyle + per-style opts,
 * source/target anchors, stroke, and marker scales.
 */
export const Manhattan: Story = {
  render: () => createContainer({ id: 'cvs-prim-router-manhattan' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const layer = new RenderLayer({ id: 'router-manhattan', options: {} });
    ...
    layer.renderer.addShape('a', {
      kind: 'rect', x: -300, y: -160, width: 80, height: 50,
    ...
    const obstacles = [
      { id: 'obstacle-1', kind: 'rect', x: -120, y:  -80, width: 70, height: 50,
        fill: { kind: 'solid', color: 0xfacc15 }, stroke: { color: 0xa16207, width: 2 } },
      { id: 'obstacle-2', kind: 'rect', x:  -40, y:   40, width: 80, height: 50,
        fill: { kind: 'solid', color: 0xfacc15 }, stroke: { color: 0xa16207, width: 2 } },
      { id: 'obstacle-3', kind: 'rect', x:  100, y:  -40, width: 70, height: 50,
        fill: { kind: 'solid', color: 0xfacc15 }, stroke: { color: 0xa16207, width: 2 } },
      { id: 'obstacle-4', kind: 'rect', x:  140, y:   90, width: 70, height: 50,
        fill: { kind: 'solid', color: 0xfacc15 }, stroke: { color: 0xa16207, width: 2 } },
      { id: 'obstacle-5', kind: 'rect', x:  -60, y: -160, width: 70, height: 50,
        fill: { kind: 'solid', color: 0xfacc15 }, stroke: { color: 0xa16207, width: 2 } },
    ];
    ...
    routerFolder.add(settings, 'stubLength', 0, 80, 1).onChange(drawEdge);
