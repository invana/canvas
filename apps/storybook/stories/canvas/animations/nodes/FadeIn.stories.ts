...
import { ElementPlugin, type CircleElementSpec, type RectElementSpec, type EllipseElementSpec } from '@invana/plugins-graph-data';
...
export const FadeIn: Story = {
  ...
  play: async () => {
    ...
    const addAll = () => {
      elements.addNode('circle',  { id: 'fa1', x: -GAP, y: 0, radius: 50, label: '500ms',  style: { fill: '#1e3a5f', stroke: '#60a5fa', strokeWidth: 2 } } as CircleElementSpec);
      elements.addNode('rect',    { id: 'fa2', x:     0, y: 0, width: 90, height: 90, label: '1000ms', style: { fill: '#1f2d3d', stroke: '#a78bfa', strokeWidth: 2 } } as RectElementSpec);
      elements.addNode('ellipse', { id: 'fa3', x:  GAP, y: 0, radiusX: 65, radiusY: 40, label: '2000ms', style: { fill: '#1f2d3d', stroke: '#f472b6', strokeWidth: 2 } } as EllipseElementSpec);
