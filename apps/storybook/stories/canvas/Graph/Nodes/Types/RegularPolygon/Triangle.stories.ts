...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/RegularPolygon/Triangle' };
export default meta;
type Story = StoryObj;

/**
 * **Triangle** regular-polygon variant (`sides: 3`). With `rotation = 0`
...
export const Triangle: Story = {
  render: () => createContainer({ id: 'graph-node-types-regpoly-triangle' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-regpoly-triangle')!;
    ...
    const graph = new GraphLayer({
      ...
      options: {
        node: {
          style: {
            shape: { kind: 'regular-polygon', sides: 3, radius: 50 },
            bgFill: 0x3b82f6,
