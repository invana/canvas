...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Star/FivePointSoft' };
export default meta;
type Story = StoryObj;

/**
 * **Soft 5-point star** variant (`points: 5`, inner/outer ratio 0.55).
 * Higher inner radius reads as a softer, "flower-like" silhouette
 * compared to the classic 0.4 ratio. Shown across the resting `default`
...
export const FivePointSoft: Story = {
  ...
  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-star-5-soft')!;
    ...
    const graph = new GraphLayer({
      ...
      options: {
        node: {
          style: {
            shape: { kind: 'star', points: 5, innerRadius: 30.25, outerRadius: 55 },
            bgFill: 0x3b82f6,
