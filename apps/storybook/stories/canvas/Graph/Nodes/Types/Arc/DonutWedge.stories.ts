...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Arc/DonutWedge' };
export default meta;
type Story = StoryObj;

/**
 * **Donut wedge** arc variant — a thin annular sector with both `innerR`
 * (> 0) and `outerR` set, sweeping a partial angular range. Shown across
 * the resting `default` plus the five canonical interaction states
 * (`hovered`, `selected`, `highlighted`, `dimmed`, `disabled`)
 * auto-merged into every layer's state catalogue.
 *
...
export const DonutWedge: Story = {
  ...
  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-arc-donut-wedge')!;
