...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Arc/QuarterPie' };
export default meta;
type Story = StoryObj;

/**
 * **Quarter pie** arc variant — a 90° wedge (`endAngle - startAngle =
 * π/2`) with `innerR: 0`. Shown across the resting `default` plus the
 * five canonical interaction states (`hovered`, `selected`,
 * `highlighted`, `dimmed`, `disabled`) auto-merged into every layer's
 * state catalogue.
 *
...
export const QuarterPie: Story = {
  ...
  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-arc-quarter-pie')!;
