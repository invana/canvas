...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Polygon/Arrow' };
export default meta;
type Story = StoryObj;

/**
 * **Arrow** polygon variant — block arrow silhouette traced through
 * seven centre-relative vertices. Shown across the resting `default`
 * plus the five canonical interaction states (`hovered`, `selected`,
 * `highlighted`, `dimmed`, `disabled`).
 */
export const Arrow: Story = {
  render: () => createContainer({ id: 'graph-node-types-polygon-arrow' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-polygon-arrow')!;
