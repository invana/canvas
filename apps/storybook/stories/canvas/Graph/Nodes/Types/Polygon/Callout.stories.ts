...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Polygon/Callout' };
export default meta;
type Story = StoryObj;
...
export const Callout: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const nodes: NodeData<TileData>[] = [
      { id: 'n-default',     position: { x: -240, y: -100 }, data: { state: 'default'     } },
      { id: 'n-hover',       position: { x:    0, y: -100 }, data: { state: 'hovered'     }, states: ['hovered']     },
      { id: 'n-selected',    position: { x:  240, y: -100 }, data: { state: 'selected'    }, states: ['selected']    },
      { id: 'n-highlighted', position: { x: -240, y:  100 }, data: { state: 'highlighted' }, states: ['highlighted'] },
      { id: 'n-dimmed',      position: { x:    0, y:  100 }, data: { state: 'dimmed'      }, states: ['dimmed']      },
