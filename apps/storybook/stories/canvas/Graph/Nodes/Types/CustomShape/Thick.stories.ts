...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/CustomShape/Thick' };
export default meta;
type Story = StoryObj;

/**
 * **Thick cross** — `CrossShape` registered at runtime under
 * `kind: 'cross'` with `arm: 50, thickness: 26`. Shown across the
 * resting `default` plus the five canonical interaction states
 * (`hovered`, `selected`, `highlighted`, `dimmed`, `disabled`).
 */
export const Thick: Story = {
  render: () => createContainer({ id: 'graph-node-types-custom-thick' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-custom-thick')!;
    ...
    const crossShape: CustomShapeOption = { kind: 'cross', arm: 50, thickness: 26 } as CustomShapeOption;
