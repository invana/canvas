...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/CustomShape/Thin' };
export default meta;
type Story = StoryObj;

/**
 * **Thin cross** — `CrossShape` registered at runtime under
 * `kind: 'cross'` with `arm: 50, thickness: 12`. Shown across the
 * resting `default` plus the five canonical interaction states
 * (`hovered`, `selected`, `highlighted`, `dimmed`, `disabled`) to prove
 * that runtime-registered shapes participate in the same state-overlay
 * pipeline as built-ins.
 */
export const Thin: Story = {
  render: () => createContainer({ id: 'graph-node-types-custom-thin' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-custom-thin')!;
    ...
    const crossShape: CustomShapeOption = { kind: 'cross', arm: 50, thickness: 12 } as CustomShapeOption;
