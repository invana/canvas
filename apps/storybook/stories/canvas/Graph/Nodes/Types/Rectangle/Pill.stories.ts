...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Rectangle/Pill' };
export default meta;
type Story = StoryObj;

/**
 * **Pill** rect variant (`cornerRadius: 32`, i.e. half the height).
 * Fully-rounded short sides — the silhouette reads as a stadium / pill.
...
export const Pill: Story = {
  render: () => createContainer({ id: 'graph-node-types-rrect-pill' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-rrect-pill')!;
    ...
    const canvasOptions = {
      layers: {
        graph: {
          node: {
            style: {
              shape: { kind: 'rect', width: 140, height: 64, cornerRadius: 32 },
              bgFill: 0x3b82f6,
