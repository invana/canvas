...
const meta: Meta = { title: 'canvas/graph/Nodes/Types/Rectangle/Sharp' };
export default meta;
type Story = StoryObj;

/**
 * **Sharp** rect variant (`cornerRadius: 0`). Hard 90° corners — the
 * default rectangle silhouette. Shown across the resting `default` plus
 * the five canonical interaction states (`hovered`, `selected`,
 * `highlighted`, `dimmed`, `disabled`).
 */
export const Sharp: Story = {
  render: () => createContainer({ id: 'graph-node-types-rrect-sharp' }),

  play: async ({ canvasElement }) => {
    const container = canvasElement.querySelector<HTMLDivElement>('#graph-node-types-rrect-sharp')!;
    ...
    const canvasOptions = {
      layers: {
        graph: {
          node: {
            style: {
              shape: { kind: 'rect', width: 140, height: 64, cornerRadius: 0 },
              bgFill: 0x3b82f6,
