...
const meta: Meta = { title: 'canvas/graph/Edges/Types/LoopCurve/Hairpin' };
export default meta;
type Story = StoryObj;

/**
 * Graph-side counterpart to
 * `Canvas/Connectors/PathStyles/LoopCurve/Hairpin`.
 *
 * `loop-curve` with the **hairpin** preset — a parallel-sided "U" where
 * `bulge ≈ width/2`, so the cubic controls sit directly above the feet
 * and the sides run parallel instead of flaring outward.
 *
...
export const Hairpin: Story = {
  render: () => createContainer({ id: 'graph-edge-loop-curve-hairpin' }),
