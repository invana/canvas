...
const meta: Meta = { title: 'canvas/graph/Edges/Types/LoopCurve/Teardrop' };
export default meta;
type Story = StoryObj;

/**
 * Graph-side counterpart to
 * `Canvas/Connectors/PathStyles/LoopCurve/Teardrop`.
 *
 * `loop-curve` with the **teardrop** preset — a slender pointed petal
 * where `radius >> bulge` and the neck `width` stays narrow, so the
 * controls converge toward the tip and the silhouette tapers.
 *
 ...
 */
export const Teardrop: Story = {
  render: () => createContainer({ id: 'graph-edge-loop-curve-teardrop' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const settings = { ...LOOP_CURVE_PRESETS.teardrop };

    const container = canvasElement.querySelector<HTMLDivElement>('#graph-edge-loop-curve-teardrop')!;
    ...
    // node template is entirely literal and lives in `canvasOptions`.
    ...
    const gui = new GUI({ title: 'loop-curve · teardrop' });
