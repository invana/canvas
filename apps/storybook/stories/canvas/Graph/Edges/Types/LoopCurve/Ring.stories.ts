...
const meta: Meta = { title: 'canvas/graph/Edges/Types/LoopCurve/Ring' };
export default meta;
type Story = StoryObj;

/**
 * Graph-side counterpart to
 * `Canvas/Connectors/PathStyles/LoopCurve/Ring`.
 *
 * `loop-curve` with the **ring** preset — `radius ≈ bulge`, so the
 * cubic Bézier closes to a near-circular loop. Reads as a true round
 * "ring" hanging off each silhouette pivot point.
 *
