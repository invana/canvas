...
export const CanvasContextMenu: Story = {
  ...
  render: () => createContainer({ id: 'cvs-canvas-ctx' }),
  play: async () => {
    ...
    const log = action('canvas:contextmenu');
    canvas.on('canvas:contextmenu', ({ position }) => {
      log({ screenX: Math.round(position.screen.x), screenY: Math.round(position.screen.y), worldX: Math.round(position.world.x), worldY: Math.round(position.world.y) });
    });
  },
};
