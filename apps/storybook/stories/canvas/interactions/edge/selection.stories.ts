...
export const EdgeSelection: Story = {
  ...
  render: () => createContainer({ id: 'cvs-edge-sel' }),
  play: async () => {
    ...
    const logSelected = action('edge:selected');
    const logDeselected = action('edge:deselected');
    canvas.on('edge:selected', ({ edge }) => {
      logSelected({ id: edge.data.id, source: edge.data.source, target: edge.data.target });
    });
    canvas.on('edge:deselected', ({ edge }) => {
      logDeselected({ id: edge.data.id, source: edge.data.source, target: edge.data.target });
    });
  },
};
