...
export const EdgeDoubleClick: Story = {
  ...
  render: () => createContainer({ id: 'cvs-edge-dbl' }),
  play: async () => {
    ...
    const log = action('edge:dblclicked');
    canvas.on('edge:dblclicked', ({ edge, position }) => {
      log({ id: edge.data.id, source: edge.data.source, target: edge.data.target, screenX: Math.round(position.screen.x), screenY: Math.round(position.screen.y), worldX: Math.round(position.world.x), worldY: Math.round(position.world.y) });
    });
  },
};
