...
export const EdgeHover: Story = {
  ...
  render: () => createContainer({ id: 'cvs-edge-hover' }),
  play: async () => {
    ...
    const logHover = action('edge:hover');
    const logHoverEnd = action('edge:hoverend');
    canvas.on('edge:hover', ({ edge, position }) => {
      logHover({ id: edge.data.id, source: edge.data.source, target: edge.data.target, worldX: Math.round(position.world.x), worldY: Math.round(position.world.y) });
    });
    canvas.on('edge:hoverend', ({ edge }) => {
      logHoverEnd({ id: edge.data.id, source: edge.data.source, target: edge.data.target });
    });
  },
};
