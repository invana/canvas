...
export const NodeHover: Story = {
  ...
  render: () => createContainer({ id: 'cvs-node-hover' }),
  play: async () => {
    ...
    const logHover = action('node:hover');
    const logHoverEnd = action('node:hoverend');
    canvas.on('node:hover', ({ node, position }) => {
      logHover({ id: node.data.id, label: node.data.label, worldX: Math.round(position.world.x), worldY: Math.round(position.world.y) });
    });
    canvas.on('node:hoverend', ({ node }) => {
      logHoverEnd({ id: node.data.id, label: node.data.label });
    });
  },
};
