...
export const NodeContextMenu: Story = {
  ...
  render: () => createContainer({ id: 'cvs-node-ctx' }),
  play: async () => {
    ...
    const log = action('node:contextmenu');
    canvas.on('node:contextmenu', ({ node, position }) => {
      log({ id: node.data.id, label: node.data.label, screenX: Math.round(position.screen.x), screenY: Math.round(position.screen.y), worldX: Math.round(position.world.x), worldY: Math.round(position.world.y) });
    });
  },
};
