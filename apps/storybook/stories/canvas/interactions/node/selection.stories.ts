...
export const NodeSelection: Story = {
  ...
  render: () => createContainer({ id: 'cvs-node-sel' }),
  play: async () => {
    ...
    const logSelected = action('node:selected');
    const logDeselected = action('node:deselected');
    canvas.on('node:selected', ({ node }) => {
      logSelected({ id: node.data.id, label: node.data.label });
    });
    canvas.on('node:deselected', ({ node }) => {
      logDeselected({ id: node.data.id, label: node.data.label });
    });
  },
};
