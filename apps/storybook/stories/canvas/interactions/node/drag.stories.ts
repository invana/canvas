...
export const NodeDrag: Story = {
  ...
  render: () => createContainer({ id: 'cvs-node-drag' }),
  play: async () => {
    ...
    const logDragStart = action('node:dragstart');
    const logDrag = action('node:drag');
    const logDragEnd = action('node:dragend');
    canvas.on('node:dragstart', ({ node, x, y }) => {
      logDragStart({ id: node.data.id, x: Math.round(x), y: Math.round(y) });
    });
    canvas.on('node:drag', ({ node, x, y }) => {
      logDrag({ id: node.data.id, x: Math.round(x), y: Math.round(y) });
    });
    canvas.on('node:dragend', ({ node, x, y }) => {
      logDragEnd({ id: node.data.id, x: Math.round(x), y: Math.round(y) });
    });
  },
};
