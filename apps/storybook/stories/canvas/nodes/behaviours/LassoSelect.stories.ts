/**
 * Canvas/Nodes/Behaviours — Lasso Select
 *
 * Demonstrates the opt-in `LassoSelectPlugin` on a small force-laid-out tree.
 * Hold the configured `trigger` modifier and drag on the canvas background to
 * draw a freeform polygon — every shape (and optionally connector) enclosed
...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Click-and-drag freeform lasso selection on the canvas background. Unions enclosed shapes (and optionally connectors) with the existing selection. Cooperates with ClickSelectPlugin when both are registered.',
      },
    },
  },
};
...
export const LassoSelect: Story = {
  ...
  play: async () => {
    ...
    // Click-select pairs nicely with the lasso — selecting a node and then
    // shift-dragging extends the selection through the same store.
    const clickSelect = new ClickSelectPlugin({
      multiple:          true,
      trigger:           ['shift'],
      state:             'selected',
      clearOnBackground: false, // lasso owns background-clear behaviour here
    });
    await canvas.plugins.register(clickSelect);

    const lasso = new LassoSelectPlugin({
      trigger:           ['shift'],
      enableElements:    ['shape', 'connector'],
      immediately:       false,
      clearOnBackground: true,
      onSelect:          snap => console.log('[lasso-select] selection:', snap),
    });
    await canvas.plugins.register(lasso);
    ...
    // ── lil-gui ────────────────────────────────────────────────────────────
    const params = {
      trigger:            'shift' as 'none' | LassoModifierKey,
      immediately:        false,
      clearOnBackground:  true,
      enableShapes:       true,
    ...
    const gui = new GUI({ container, title: 'Lasso Select' });
    gui.domElement.style.cssText = 'position:absolute;top:10px;right:10px;z-index:100;';

    const sync = () => {
      const trigger: LassoModifierKey[] =
      ...
      // multi-selects when the lasso trigger changes.
