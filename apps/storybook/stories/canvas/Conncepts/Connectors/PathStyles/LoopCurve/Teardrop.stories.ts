...
const meta: Meta = { title: 'canvas/concepts/Connectors/PathStyles/LoopCurve/Teardrop' };
export default meta;
type Story = StoryObj;

/**
 * `loop-curve` with the **teardrop** preset — a slender, pointed petal
 * (`radius >> bulge`, narrow `width`) where the cubic controls converge
 * toward the tip, producing a long axis with a pinched belly.
 *
...
export const Teardrop: Story = {
  render: () => createContainer({ id: 'cvs-prim-loop-curve-teardrop' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-prim-loop-curve-teardrop')!;
    ...
    const layer = new RenderLayer({ id: 'loop-curve-teardrop', options: {} });
