...
const meta: Meta = { title: 'canvas/Concepts/Connectors/PathStyles/LoopCurve/Ring' };
export default meta;
type Story = StoryObj;

/**
 * `loop-curve` with the **ring** preset — a near-circular loop where
 * `radius ≈ bulge` so the belly is about as tall as it is wide, and
 * the neck is wide enough that the loop reads as a continuous ring
 * rather than a pinched petal.
 *
...
export const Ring: Story = {
  render: () => createContainer({ id: 'cvs-prim-loop-curve-ring' }),
  ...
  play: async ({ canvasElement }) => {
    ...
    const container = canvasElement.querySelector<HTMLDivElement>('#cvs-prim-loop-curve-ring')!;
    ...
    const layer = new RenderLayer({ id: 'loop-curve-ring', options: {} });
    ...
    const gui = new GUI({ title: 'loop-curve · ring' });
