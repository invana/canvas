...
const meta: Meta = { title: 'canvas/concepts/Decorations/Connectors/FlyMarkerConnector' };
export default meta;
type Story = StoryObj;

/**
 * Connector decoration that flies a single marker along the routed path.
 * Demonstrated on two edges using the `straight` router with `normal` and
 * `bezier` path styles to show that the marker follows whatever curve the
...
export const FlyMarkerConnectorStory: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const layer = new RenderLayer({ id: 'fly-marker-connector', options: {} });
    ...
    const apply = () => {
      ...
      layer.renderer.setDecoration('a-to-b', 'fly-marker-connector', {
        kind: 'fly-marker-connector',
        style,
      });
      layer.renderer.setDecoration('c-to-d', 'fly-marker-connector', {
        kind: 'fly-marker-connector',
        style,
      });
    };
