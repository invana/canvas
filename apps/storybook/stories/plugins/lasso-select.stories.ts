...
export const LassoSelect: Story = {
  name: 'Lasso Select',
  render: () => {
    const div = createContainer({ id: 'cvs-lasso-select' });
    div.style.position = 'relative';
    return div;
  },
  play: async () => {
    const container = document.getElementById('cvs-lasso-select');
    if (!container) return;
    ...
    await canvas.registerPlugin(lassoPlugin);
    ...
    gui.add(settings, 'clearOnBackground').name('Clear on Background').onChange(sync);
