 ...
 *   - **d3-sankey** — flow columns; rewrites nodes into rects and edges into
 *     ribbons sized by `data.value`.
 *   - **ELK** (layered / tree) and **geometric** (grid / circular) for contrast.
 *
 * Switching away from a geometry-rewriting layout (pack / sunburst / sankey)
 * leaves the morphed shapes in the store — hit **Reset** to return to plain
 * circles. Cyclic graphs live in the sibling `CyclicLayouts` story.
 *
...
const EXTRA_LAYOUTS: readonly LayoutEntry[] = [
  {
    id: 'hierarchy-tree',
    label: 'Tree (d3)',
    make: () => new D3HierarchyLayout({ id: 'hierarchy-tree', targetLayerId: LAYER_ID, mode: 'tree', ...GLIDE }),
  },
  {
    id: 'hierarchy-cluster',
    label: 'Cluster (d3)',
    make: () => new D3HierarchyLayout({ id: 'hierarchy-cluster', targetLayerId: LAYER_ID, mode: 'cluster', ...GLIDE }),
  },
  {
    id: 'hierarchy-radial-tree',
    label: 'Radial Tree (d3)',
    make: () =>
      new D3HierarchyLayout({ id: 'hierarchy-radial-tree', targetLayerId: LAYER_ID, mode: 'radial-tree', radius: 300, ...GLIDE }),
  },
  {
    id: 'hierarchy-radial-cluster',
    label: 'Radial Cluster (d3)',
    make: () =>
      new D3HierarchyLayout({ id: 'hierarchy-radial-cluster', targetLayerId: LAYER_ID, mode: 'radial-cluster', radius: 300, ...GLIDE }),
  },
  {
    id: 'hierarchy-pack',
    label: 'Pack (d3)',
  ...
  },
  {
    id: 'sankey',
    label: 'Sankey (d3)',
    // Sankey snaps (it replaces node rects + edge ribbons), so no GLIDE. Centre
    // the [800×500] diagram on the world origin for a natural fit.
    make: () =>
      new D3SankeyLayout({ id: 'sankey', targetLayerId: LAYER_ID, size: [800, 500], center: { x: -400, y: -250 } }),
  },
  {
    id: 'elk-layered',
    label: 'Layered (ELK)',
