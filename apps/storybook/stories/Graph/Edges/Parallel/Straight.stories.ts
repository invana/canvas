...
export const Straight: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const settings = { nodeKind: 'circle', anchor: 'boundary', count: 7, spacing: 26 };
    ...
    const patchAllEdges = () => {
      ...
      const nx = -dy / len;
      const ny =  dx / len;
      // Side derivation is only consumed by the port anchors; computed
      // unconditionally to keep the closure simple.
