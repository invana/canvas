...
/**
 * `count` parallel `manhattan` edges between two nodes. Router `manhattan`
 * (obstacle-aware A* on top of orth) + pathStyle `normal`. With no
 * obstacles in the way, the routes look identical to `orth`; the demo
 * is here so the variant has a home and the waypoint-scheme switch is
 * easy to compare side-by-side.
 *
 *  - **Diagonal endpoints** → 1 mid-waypoint, 2-corner Z per edge.
    ...
    const patchAllEdges = () => {
      ...
      for (let i = 0; i < settings.count; i++) {
        ...
        graph.store.updateEdge(`e${i}`, {
          style: { shape: { pathType: 'manhattan', waypoints } },
        });
      }
    };
