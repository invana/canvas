/**
 * HTMLLabel - Rich HTML text label using PixiJS HTMLText
 * Perfect text rendering using browser's native text engine
 * Better quality but slightly more expensive than TextLabel
 */
...
export class HTMLLabel {
  ...
  constructor(text: string, style?: Partial<LabelStyle>) {
    ...
    this._container.label = 'html-label';
    ...
    // Create HTMLText for crisp text rendering using browser's native text engine
  ...
  /**
   * Set HTML content directly (supports HTML tags)
   */
  ...
  private _createTextStyle(): HTMLTextStyle {
