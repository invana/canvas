/**
 * Label - Unified label interface
 * 
 * This is a facade that provides a common interface for both:
 * - TextLabel: High-performance native PixiJS text (default)
 * - HTMLLabel: Rich HTML text with browser-native rendering
 * 
 * For direct access, import TextLabel or HTMLLabel explicitly.
 */
...
export interface LabelOptions {
  /** Label type: 'text' for performance, 'html' for quality */
  type?: LabelType;
}

/**
 * Label class - High-performance native text label
 * Uses PixiJS Text with optimized resolution settings
 */
export class Label {
  ...
  constructor(text: string, style?: Partial<LabelStyle>) {
    ...
    // Use high resolution: at least 2x DPR for crisp rendering at all zoom levels
