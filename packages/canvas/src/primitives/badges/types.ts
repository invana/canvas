 ...
 * Badge type surface.
 *
 * A badge is a regular shape registered as a "follower" of a host primitive
 * (shape or connector) — its `(x, y)` is computed from the host's geometry
 * plus a placement anchor. Because a badge is just a shape under the hood
 * (registered with id `${hostId}:${slot}`), it picks up every existing
 * capability for free: any registered shape kind as a plate, any
 * `ShapeFillLayer` as content (solid / image / glyph / svg / svg-url), any
 * registered decoration via `setDecoration`, and any shape effect via
 * `setEffect`.
 *
 * **Shape host** — placement resolves against the host's AABB via one of
 *   the eight {@link NamedBadgePlacement} points (or a raw `{x, y}`).
 * **Connector host** — placement resolves along the host's routed path via
 *   {@link ConnectorBadgePlacement} (`'start' | 'middle' | 'end' | number`).
 *   `pathOffset` shifts along the tangent; `autoRotate` follows the path.
 *
 * See `badges-edges-plan.md` at the repo root for the full design.
 */

import type { BaseShapeSpec, DecorationSpec, EffectSpec } from '../types';
