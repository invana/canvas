 ...
 *
 * Opts:
 *  - `side: 'left' | 'right' | 'top' | 'bottom'` — which face to attach to.
 *    Required; throws if omitted.
...
export const edgePortAnchor: IAnchor = (endpoint, _fromPoint, ctx) => {
  ...
  if (!opts || !opts.side) {
    throw new Error(
      `edgePortAnchor: shape "${endpoint.shapeId}" is missing the required \`side\` opt`,
    );
  }
