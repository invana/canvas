 ...
 *
 * `opts.samples` controls polyline resolution. Default `16` — enough for
 * smooth-looking strokes at typical screen scales without over-sampling.
 *
 * Returned polyline includes both endpoints as the first and last entries.
 */
...
export const bezierRouter: IRouter = (a, b, opts) => {
...
function handleLength(a: Point, b: Point): number {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  // 1/3 of endpoint distance is a common heuristic for "natural" curvature.
...
function applyTangent(p: Point, tangent: Vec2 | undefined, length: number): Point {
  ...
  // Default: horizontal control offset (positive length pushes "outward").
