/**
 * Built-in **composite node types** — ready-made node layouts built on the
 * engine's domain-free `composite` shape, the composite counterpart to the
 * geometry node types in `BUILT_IN_STRUCTURES`.
 *
 * Each type is a {@link CompositeCard} **subclass** driven by a typed **spec**
 ...
 * node. Override the `protected` section methods only for structural changes.
 * Each type also exports a stock convenience function.
 *
 * ```ts
...
export { SchemaTableCard, schemaTableCard, SCHEMA_TABLE_CARD_DEFAULTS, type SchemaTableCardSpec, type SchemaTableCardOptions } from './schemaTableCard';
...
export { IDCard, idCard, ID_CARD_DEFAULTS, type IDCardSpec } from './idCard';
export { OrganisationCard, organisationCard, ORGANISATION_CARD_DEFAULTS, type OrganisationCardSpec } from './organisationCard';
export { ProductCard, productCard, PRODUCT_CARD_DEFAULTS, type ProductCardSpec } from './productCard';
export { EventCard, eventCard, EVENT_CARD_DEFAULTS, type EventCardSpec } from './eventCard';
export { iconifyUrl, chip, metaRow, CARD_BG, CARD_STROKE, type ChipOptions, type MetaRowOptions } from './shared';
export type {
  SchemaField,
  SchemaTableData,
  UserCardData,
