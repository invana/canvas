...
Visual **authoring tools** for the canvas — currently the free-form **node card / template designer**. Split out from `@invana/canvas-ui` so it's *opt-in*: most consumers only **render** templates (that needs nothing here — just `@invana/graph`'s `FreeformStructure` + `compileFreeform`); only apps that let end-users **design** their own node cards install this package.
...
- **No tests** (per repo convention); verify via Storybook (`Usecases/Card Designer Studio`). Don't add stories unless asked.
