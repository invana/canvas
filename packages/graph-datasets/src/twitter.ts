 ...
 *
 * Authored in the package's property-graph shape (`{ id, label, properties }`);
 * map `label`→`type`, `properties`→`data` at `setData` time (see CLAUDE.md).
 *
 * Node labels: `User`, `Tweet`, `Comment`, `Hashtag`, `Retweet`.
 * Edge labels: `POSTED`, `TAGGED`, `MENTIONS`, `WROTE`, `REPLY_TO`,
 ...
 *
 * @example
 * import { twitterActivity, generateTwitterActivity } from '@invana/graph-datasets';
...
export function generateTwitterActivity(opts: TwitterDatasetOptions = {}): TwitterGraphData {
  ...
  const link = (source: string, target: string, label: string): void => {
    edges.push({ id: `e${eid++}`, label, source, target });
  };
  ...
  const propsOf = (id: string) =>
    nodes.find((n) => n.id === id)!.properties as { name: string; handle: string; avatar: string };
  ...
  for (let i = 0; i < hashtags; i++) {
    ...
    const id = `h${i}`;
    nodes.push({ id, label: 'Hashtag', properties: { tag, label: `#${tag}`, uses: 1 + int(60) } });
    tagIds.push(id);
  }
  ...
  for (let i = 0; i < comments; i++) {
    ...
    nodes.push({
      id,
      label: 'Comment',
      properties: { text: pick(COMMENTS), author: a.name, handle: a.handle, time: shortTime(ts), ts },
    });
  ...
  for (let i = 0; i < retweets; i++) {
    ...
    nodes.push({
      id,
      label: 'Retweet',
      properties: { by: a.handle, label: `RT ${a.handle}`, time: shortTime(ts), ts },
    });
