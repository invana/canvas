// ── ShapePool ─────────────────────────────────────────────────────────────────
// Stores ALL shape objects (including off-screen ones) and maintains an RBush
...
/**
 * `ShapePool` stores `ShapeObject` instances and maintains an RBush R-tree
 * for O(log n) viewport culling and pointer hit-testing.
 *
 * @remarks
 * Use a **separate** `ShapePool` instance for shape elements and for connector
...
export class ShapePool {
  ...
  /**
   * Return all `ShapeObject`s whose bounding box intersects the given AABB.
   */
   ...
   */
  hitTest(wx: number, wy: number): ShapeObject | null {
  ...
  /** All bounding boxes — used by `ShapesPlugin.fitContent()`. */
  allBBoxes(): BBox[] {
    return Array.from(this._objects.values()).map(o => o.getBBox());
  }

  /** Iterable over all stored `ShapeObject`s. */
  ...
  private _entry(obj: ShapeObject): RBushEntry {
    return { ...obj.getBBox(), id: obj.id };
  }
}

// ── Backward-compatibility alias ─────────────────────────────────────────────
/** @deprecated Use {@link ShapePool} instead. */
export { ShapePool as GraphPool };
