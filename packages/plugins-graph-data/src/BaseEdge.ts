 ...
 *   }
 * }
 * graphPlugin.registerEdge('curved', CurvedConnector);
 * ```
 *
...
export abstract class BaseEdge<S extends BaseEdgeSpec = BaseEdgeSpec> {
   ...
   * Modified by {@link GraphPlugin.updateEdge} — do not mutate directly.
   */
  spec: S;
  ...
  /**
   * Called by {@link GraphPlugin} to inject the router registry so that
  ...
  /**
   * Called by {@link GraphPlugin} to inject the marker registry so that
  ...
  /**
   * Called by {@link GraphObject} to trigger a redraw when this element is
   * marked dirty.
   * @internal
   */
  _onDirty?: () => void;
