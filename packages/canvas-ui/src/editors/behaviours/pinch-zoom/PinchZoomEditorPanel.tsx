...
import { pinchZoomFields } from './fields';
import type { PinchZoomFields, PinchZoomFormState } from './types';

export interface PinchZoomEditorPanelProps {
  /**
 ...
 */
export function PinchZoomEditorPanel({
  defaults = {},
  fields = pinchZoomFields,
  onSubmit,
  submitLabel = 'Apply',
}: PinchZoomEditorPanelProps) {
  const form = useForm<PinchZoomFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as PinchZoomFields | undefined;
