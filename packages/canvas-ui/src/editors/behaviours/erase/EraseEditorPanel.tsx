...
import { eraseFields } from './fields';
import type { EraseFields, EraseFormState } from './types';

export interface EraseEditorPanelProps {
  /**
 ...
 */
export function EraseEditorPanel({
  defaults = {},
  fields = eraseFields,
  onSubmit,
  submitLabel = 'Apply',
}: EraseEditorPanelProps) {
  const form = useForm<EraseFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as EraseFields | undefined;
