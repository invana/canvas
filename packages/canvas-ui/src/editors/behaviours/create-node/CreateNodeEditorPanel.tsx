...
import { createNodeFields } from './fields';
import type { CreateNodeFields, CreateNodeFormState } from './types';

export interface CreateNodeEditorPanelProps {
  /**
...
export function CreateNodeEditorPanel({
  ...
  const form = useForm<CreateNodeFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as CreateNodeFields | undefined;
