...
import { dragNodeFields } from './fields';
import type { DragNodeFields, DragNodeFormState } from './types';

export interface DragNodeEditorPanelProps {
  /**
 ...
 */
export function DragNodeEditorPanel({
  defaults = {},
  fields = dragNodeFields,
  onSubmit,
  submitLabel = 'Apply',
}: DragNodeEditorPanelProps) {
  const form = useForm<DragNodeFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as DragNodeFields | undefined;
