...
import { colorByLabelFields } from './fields';
import type { ColorByLabelFields, ColorByLabelFormState } from './types';

export interface ColorByLabelEditorPanelProps {
  /**
   ...
   * to reload.
   */
  defaults?: ColorByLabelFields;
  /**
   * The form schema. Either a static `FieldConfig[]` or a function of the
   * current values. Defaults to {@link colorByLabelFields}.
   */
  fields?: FieldConfig[] | ((values: ColorByLabelFields) => FieldConfig[]);
  /**
   ...
   */
  onSubmit: (values: ColorByLabelFields) => void;
  /** Submit button label. Default `'Apply'`. */
  submitLabel?: string;
}

/**
 * Self-contained, engine-agnostic settings form for `ColorByLabelBehaviour`.
 *
 ...
 * engine reference and does no commit — the `ColorByLabelBehaviourOptions ⇄
 * form-fields` mapping (`optionsToForm` / `formToOptions`) is the consumer's plug-in.
 */
export function ColorByLabelEditorPanel({
  defaults = {},
  fields = colorByLabelFields,
  onSubmit,
  submitLabel = 'Apply',
}: ColorByLabelEditorPanelProps) {
  const form = useForm<ColorByLabelFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as ColorByLabelFields | undefined;
