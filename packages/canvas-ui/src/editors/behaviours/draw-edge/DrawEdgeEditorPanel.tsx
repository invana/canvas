...
import { drawEdgeFields } from './fields';
import type { DrawEdgeFields, DrawEdgeFormState } from './types';

export interface DrawEdgeEditorPanelProps {
  /**
 ...
 */
export function DrawEdgeEditorPanel({
  defaults = {},
  fields = drawEdgeFields,
  onSubmit,
  submitLabel = 'Apply',
}: DrawEdgeEditorPanelProps) {
  const form = useForm<DrawEdgeFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as DrawEdgeFields | undefined;
