...
import { dragShapeFields } from './fields';
import type { DragShapeFields, DragShapeFormState } from './types';

export interface DragShapeEditorPanelProps {
  /**
 ...
 */
export function DragShapeEditorPanel({
  defaults = {},
  fields = dragShapeFields,
  onSubmit,
  submitLabel = 'Apply',
}: DragShapeEditorPanelProps) {
  const form = useForm<DragShapeFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as DragShapeFields | undefined;
