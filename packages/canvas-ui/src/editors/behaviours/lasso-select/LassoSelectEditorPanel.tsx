...
import { lassoSelectFields } from './fields';
import type { LassoSelectFields, LassoSelectFormState } from './types';

export interface LassoSelectEditorPanelProps {
  /**
 ...
 */
export function LassoSelectEditorPanel({
  defaults = {},
  fields = lassoSelectFields,
  onSubmit,
  submitLabel = 'Apply',
}: LassoSelectEditorPanelProps) {
  const form = useForm<LassoSelectFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as LassoSelectFields | undefined;
