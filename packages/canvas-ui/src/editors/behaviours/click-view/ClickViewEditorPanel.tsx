...
import { clickViewFields } from './fields';
import type { ClickViewFields, ClickViewFormState } from './types';

export interface ClickViewEditorPanelProps {
  /**
 ...
 */
export function ClickViewEditorPanel({
  defaults = {},
  fields = clickViewFields,
  onSubmit,
  submitLabel = 'Apply',
}: ClickViewEditorPanelProps) {
  const form = useForm<ClickViewFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as ClickViewFields | undefined;
