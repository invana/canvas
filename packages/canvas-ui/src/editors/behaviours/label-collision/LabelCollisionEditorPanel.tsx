...
import { labelCollisionFields } from './fields';
import type { LabelCollisionFields, LabelCollisionFormState } from './types';

export interface LabelCollisionEditorPanelProps {
  /**
 ...
 */
export function LabelCollisionEditorPanel({
  defaults = {},
  fields = labelCollisionFields,
  onSubmit,
  submitLabel = 'Apply',
}: LabelCollisionEditorPanelProps) {
  const form = useForm<LabelCollisionFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as LabelCollisionFields | undefined;
