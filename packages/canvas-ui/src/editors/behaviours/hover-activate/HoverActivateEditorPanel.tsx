...
import { hoverActivateFields } from './fields';
import type { HoverActivateFields, HoverActivateFormState } from './types';

export interface HoverActivateEditorPanelProps {
  /**
 ...
 */
export function HoverActivateEditorPanel({
  defaults = {},
  fields = hoverActivateFields,
  onSubmit,
  submitLabel = 'Apply',
}: HoverActivateEditorPanelProps) {
  const form = useForm<HoverActivateFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as HoverActivateFields | undefined;
