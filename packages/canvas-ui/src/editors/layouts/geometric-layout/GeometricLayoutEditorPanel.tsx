...
import { geometricLayoutFields } from './fields';
import type { GeometricLayoutFields, GeometricLayoutFormState } from './types';

export interface GeometricLayoutEditorPanelProps {
  /**
 ...
 */
export function GeometricLayoutEditorPanel({
  defaults = {},
  fields = geometricLayoutFields,
  onSubmit,
  submitLabel = 'Apply',
}: GeometricLayoutEditorPanelProps) {
  const form = useForm<GeometricLayoutFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function (drives
  // the per-mode numerics off the watched `mode`).
  const values = useWatch({ control, name: 'options' }) as GeometricLayoutFields | undefined;
