...
import { d3SankeyLayoutFields } from './fields';
import type { D3SankeyLayoutFields, D3SankeyLayoutFormState } from './types';

export interface D3SankeyLayoutEditorPanelProps {
  /**
 ...
 */
export function D3SankeyLayoutEditorPanel({
  defaults = {},
  fields = d3SankeyLayoutFields,
  onSubmit,
  submitLabel = 'Apply',
}: D3SankeyLayoutEditorPanelProps) {
  const form = useForm<D3SankeyLayoutFormState>({ defaultValues: { options: defaults } });
  const { control, getValues } = form;

  // Recompute fields from the live values when `fields` is a function.
  const values = useWatch({ control, name: 'options' }) as D3SankeyLayoutFields | undefined;
