...
export interface BackgroundOptions {
   ...
   * background feels like it's part of the world (infinite scrolling grid).
   * When false (default) the pattern is fixed to the screen.
   */
...
export class BackgroundPlugin implements CanvasPlugin {
  ...
  // Current camera state for followCamera mode
  private _camX = 0;
  private _camY = 0;
  private _camScale = 1;

  constructor(options: BackgroundOptions & { key?: string } = {}) {
    const { key, ...bgOptions } = options;
    this.id = key ?? 'background';
    this._options = { ...DEFAULT_OPTIONS, ...bgOptions };
  }
  ...
  private _syncTileTransform(): void {
    ...
    this._tilingSprite.tilePosition.set(this._camX % (this._options.spacing * s), this._camY % (this._options.spacing * s));
  }
  ...
  private _render(): void {
    ...
    // Apply current camera state if followCamera is already active
    this._syncTileTransform();
  }
