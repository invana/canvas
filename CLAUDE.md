...
| `packages/canvas` | `@invana/canvas` | the **renderer-agnostic orchestrator** — `Canvas` (builds the concrete context, owns the tick + renderer lifecycle), `CanvasConfig`, built-in layers (`Background`/`DevInfo`/`LayersPanel` + the `World`/`Screen` bases) and behaviours (drag-pan/zoom/LOD…), and `io/` (raster + SVG-document + state export/import). **Re-exports the whole `@invana/canvas-core` surface and the kernel's spec vocabulary + picking** so consumers keep importing everything from here. **Imports no drawing library, and no third-party library at all** — its dependencies are `@invana/canvas-core` + `@invana/canvas-store` + `@invana/renderer-pixijs`. The backend is a **required `dependency`, imported at module scope** (`file:packages/canvas/src/engine/Canvas.ts#L42`) so `init()` works with no configuration; swapping it means passing your own `IRenderer` via `opts.renderer`, not omitting the package. | — (deps: `@invana/canvas-core`, `@invana/canvas-store`, `@invana/renderer-pixijs`) |
| `packages/renderer-pixijs` | `@invana/renderer-pixijs` | **the PixiJS drawing backend — the only package in the repo that imports a drawing library.** Implements `IRenderer` / `ISurface` / `IElementRenderer` / `ICameraBinding`: the pixi `Application`, the viewport binding, all of `primitives/` (shapes, connectors, decorations, effects, markers, paint), textures and fonts. Enforced by `pnpm check-boundaries` (incl. the `backend-detach` row: it must NOT import `@invana/canvas`). See its `CLAUDE.md`. | `pixi.js`, `pixi-viewport` (peers: `@invana/canvas-core`, `@invana/canvas-store`) |
| `packages/canvas-telemetry-otel` | `@invana/canvas-telemetry-otel` | **the OpenTelemetry adapter for the kernel's telemetry ports — the only *package* that imports an OTel SDK** (one stray remains in `apps/`: `apps/storybook/stories/canvas-store/otel.ts` hand-rolls a tracer that predates this package). `otelTelemetry(opts)` turns an endpoint + stream flags into the `CanvasTelemetryConfig` (`traces` / `metrics` / `logging`) that `new Canvas({ telemetry })` and `createCanvasStore({ telemetry })` accept, backed by real OTLP/**HTTP** exporters (browsers can't speak OTLP/gRPC) to HyperDX or any collector. **Opt-in and inert by default** — nothing in the engine imports it; you pass its result in. The kernel stays vendor-free (it only declares the `tracer`/`meter`/`logger` ports). See its `CLAUDE.md`. | `@opentelemetry/*` (api, api-logs, sdk-trace-web/base, sdk-metrics, sdk-logs, the three otlp-http exporters, resources, semantic-conventions) (peer: `@invana/canvas-store`) |
| `packages/graph` | `@invana/graph` | graph domain on top of the engine — `GraphLayer`, `MiniMapLayer`, `GraphCanvas`, hover/click/lasso/brush/drag/select/context-menu/etc. behaviours, `OneShotPositionLayout` base | `rbush` (label-collision index) (peer: `@invana/canvas`) |

#### Layouts (each wraps one algorithm; peer on `@invana/canvas-core` + `@invana/graph`)
...
@invana/canvas-core  (the dependency-free FLOOR — imports NOTHING. The vocabulary: specs +
   ...
   │
@invana/canvas-store  (the state MACHINERY — the ONLY home of zustand/immer: the immer
   ▲                   patch engine + adapters behind core's port, history, picking (rbush —
   │                   the picking index is store-only, but rbush itself is NOT exclusive to
   │                   this package: @invana/graph uses it too for label collision),
   │                   telemetry, and createCanvasStore { view, data, events }. Re-exports the
   │                   core vocabulary for kernel consumers.)
   │
   ├── @invana/canvas-telemetry-otel  (the OTel ADAPTER — peer: canvas-store; the ONLY importer
   │                                   of @opentelemetry/*. otelTelemetry() → CanvasTelemetryConfig
   │                                   for `new Canvas({ telemetry })`. OPT-IN: nothing imports it,
   │                                   the consumer passes it in, so the kernel stays vendor-free.)
   │
@invana/canvas  (renderer-AGNOSTIC orchestrator: Canvas + built-in layers/behaviours + io.
   ...
   │
   ├── @invana/renderer-pixijs   (the pixi BACKEND — peer: canvas-core + canvas-store; owns pixi.js
   │                              + pixi-viewport. canvas takes it as a REQUIRED dependency and
   │                              imports it at module scope; `opts.renderer` swaps the backend.)
   ├── @invana/graph                         (peer: canvas; dep: canvas-store)
   │      ▲
   │      ├── graph-layout-*                 (peer: canvas-core, graph) — d3-force/elkjs/d3-hierarchy/d3-sankey/geometric
...
1. **Renderer** — a drawing library may be imported *only* inside a backend package. If the engine
   ...
   `packages/canvas-core/src/contracts/IRenderer.ts` and implement it there. See
...
Rules implied by this graph: cross-package engine deps are **`peerDependencies`** (+ mirrored in `devDependencies` for local builds), never plain `dependencies` — except `canvas-react`→`@invana/themes`, the foundational **`@invana/canvas-store`** dependency (a normal `dependency`, not a peer — the shared kernel below the engine), and app deps. Keep all in-repo packages on the **same version** when bumping. A new **layout** package peers on `@invana/canvas-core` (+ `@invana/graph`); a new **layer** package peers on `@invana/canvas` (+ `@invana/graph` unless it's canvas-only like maplibre) and lists its single algorithm lib as a 3rd-party `dependency`. After adding a package, also add it to `apps/storybook`'s deps if it gets a story.
...
16. **At most one `export *` per barrel, and only for the specs vocabulary** (`…/specs`) — every other export is named. Two stars in one barrel silently *omit* colliding names; and a name must never reach a barrel by two routes (explicit + star — the explicit one silently shadows). The surface itself is pinned by `pnpm check-api-surface` (part of `pnpm lint`): it diffs each engine barrel's **built** `.d.ts` export list (stars resolved) against `api/<pkg>.surface.txt`, so any surface change — starred or explicit, addition or deletion — fails until the snapshot is regenerated (`pnpm build && node scripts/check-api-surface.mjs --write`) in the same change. See `docs/rfcs/feat/2026-08-12-star-exports-hide-surface-drift.md`.
...
- **Telemetry is a port decorator** (`withTelemetry`), not a zustand middleware — transfers across the backend swap; scoped to the `view` reactive store, never the bulk data hot path. **The kernel ships the ports, never a vendor SDK**: it declares `tracer` / `meter` / `logger` and no-ops when they're absent. The concrete OpenTelemetry wiring lives in the separate opt-in **`@invana/canvas-telemetry-otel`** — `new Canvas({ telemetry: otelTelemetry({ endpoint, traces, metrics, logging }) })` — so a consumer who wants no observability pulls in no OTel bytes. Don't import `@opentelemetry/*` anywhere else.
